...
// Deliberately a separate implementation from AudioPage/VideoPage, not a
// shared parameterized component -- same reasoning as audio/page.tsx's
// docstring: this flow (transcribe -> scene review -> render) is different
// enough, and should be free to diverge without the others dragging on it.
export default function VideoGenPage() {
  ...
  const loadJobs = useCallback(() => {
    apiFetch<{ jobs: Job[]; total: number }>("/api/jobs?job_type=video_gen&limit=50")
  ...
  async function handleUpload(e: React.FormEvent) {
    ...
    try {
      ...
      await apiFetch("/api/generate_video", { method: "POST", body: formData });
  ...
  return (
    <div className="w-full px-6 py-10">
      <p className="font-sans text-xs font-semibold text-accent">Audio → Video</p>
