...
function StatusBadge({ job }: { job: Job }) {
  const styles: Record<Job["status"], string> = {
    queued: "bg-brand-navy-soft text-brand-navy dark:text-foreground",
  ...
  return (
    <div className="flex flex-1 flex-col">
      ...
      <div className="mx-auto w-full max-w-3xl flex-1 px-6 py-10">
        <h1 className="text-2xl font-bold tracking-tight text-brand-navy dark:text-foreground">Dashboard</h1>
        <p className="mt-1 text-sm text-muted">Submit a video and track it through to a finished document.</p>

        <form
          onSubmit={handleUpload}
          className="mt-8 rounded-2xl border border-brand-border bg-surface p-6 shadow-soft"
        >
          <h2 className="text-sm font-semibold text-foreground">Convert a video</h2>
          <p className="mt-1 text-xs text-muted">MP4, MOV, MKV, WebM, AVI, or M4V, up to 90 minutes.</p>
        ...
        <div className="mt-10">
          ...
          {jobs === null ? (
            ...
            <p className="mt-3 rounded-2xl border border-dashed border-brand-border p-6 text-center text-sm text-muted">
              No jobs yet -- upload a video above to get started.
            </p>
          ) : (
